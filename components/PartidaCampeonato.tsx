  ...
  return (
    <div className="mt-3">
      <div className="flex items-center gap-2">
        <span className="font-pixel text-[8px] text-suave">PARTIDAS</span>
      ...
      {proxima > 0 && <p className="mt-1 text-right text-[9px] text-suave">🕒 +1 partida em {fmt(proxima)}</p>}

      {podeJogar ? (
        <Link href={href} className={`mt-2 block border-2 px-4 py-2.5 text-center font-pixel text-[10px] transition ${btn}`}>
          ⚔️ JOGAR PARTIDA
        </Link>
      ) : (
        <p className="mt-2 border-2 border-borda px-3 py-2 text-center text-[11px] text-suave">
