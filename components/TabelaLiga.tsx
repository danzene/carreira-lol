...
export default function TabelaLiga({ liga }: { liga: LigaState }) {
  ...
  return (
    <div className="border-2 border-borda bg-painel">
      <div className="grid grid-cols-[28px_1fr_32px_32px] gap-1 border-b-2 border-borda px-3 py-2 font-pixel text-[8px] text-suave">
        <span>#</span>
        <span>TIME</span>
      ...
      {ordenada.map((c, i) => {
        ...
        return (
          ...
          >
            <span className={`font-pixel text-[10px] ${i < 4 ? "text-ciano" : "text-suave"}`}>{i + 1}</span>
      ...
      <p className="border-t-2 border-borda px-3 py-1.5 text-[10px] text-suave">Top 4 (ciano) vão aos playoffs.</p>
    </div>
  );
}
