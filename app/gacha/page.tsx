...
import { efeitoLendas, type ResultadoPuxada } from "@/engine/gacha";
        ...
        {LENDAS.map((l) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col gap-1 border-2 p-2 text-left transition ${tem ? "" : "opacity-40"} ${eq ? "bg-ciano/10" : "bg-painel"}`}
            >
              <div className="flex items-center justify-between">
                <span className="font-pixel text-[9px]" style={{ color: tem ? info.cor : "#9a90c0" }}>
                  {"★".repeat(l.raridade)}
                </span>
                {tem && <span className="text-[8px] text-suave">{eq ? "✓ EQUIP." : `Nv.${tem.nivel}`}</span>}
              </div>
              <p className="truncate text-xs text-texto">{tem ? l.nome : "???"}</p>
              {tem && (
                <div className="flex flex-wrap gap-1">
                  {tem.substats.map((s, j) => (
                    <span key={j} className="border border-borda px-1 text-[8px] text-suave">
