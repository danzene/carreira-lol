...
function LinhaLado({ lado, destaque }: { lado: LadoDuelo; destaque: boolean }) {
  return (
    <div className={`flex-1 border-2 p-3 text-center ${destaque ? "border-ciano bg-ciano/10" : "border-borda bg-painel"}`}>
      <p className="font-pixel text-[10px] text-texto">{lado.nome}</p>
      <p className="mt-1 text-[9px] text-suave">
        {lado.rota} · {lado.elo}
      </p>
      <p className="mt-2 text-2xl font-bold text-rosa">{lado.abates}</p>
      <p className="text-[8px] text-suave">abates</p>
      <p className="mt-1 text-[9px] text-ciano">força {lado.forca} · nota {lado.nota}</p>
      {destaque && <p className="mt-1 font-pixel text-[8px] text-emerald-400">VENCEDOR 🏆</p>}
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={onFechar}>
      ...
      >
        ...
        <div className="flex items-stretch gap-2">
          ...
          <div className="flex items-center font-pixel text-[10px] text-suave">VS</div>
          <LinhaLado lado={res.b} destaque={res.b.venceu} />
        </div>
        <div className="mt-4 space-y-1 border-2 border-borda bg-painel p-3 text-[10px] text-suave">
          {res.log.map((l, i) => (
            <p key={i}>{l}</p>
          ))}
        </div>
        <button
          type="button"
          onClick={onFechar}
          className="mt-4 w-full border-2 border-ciano py-2 font-pixel text-[10px] text-ciano transition hover:bg-ciano hover:text-fundo"
        >
          FECHAR
        </button>
      </div>
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-5 px-4 py-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[10px] text-suave">Assíncrono e determinístico — enfrente o snapshot de outros players</p>
        </div>
      ...
      <div className="flex items-center justify-between border-2 border-ciano/40 bg-ciano/5 p-4">
        <div>
          <p className="font-pixel text-[11px] text-ciano">{nick}</p>
          <p className="mt-1 text-[9px] text-suave">
      ...
      {semCarreira && (
        <div className="border-2 border-rosa/50 bg-rosa/5 p-3 text-center text-[10px] text-suave">
      ...
      <section className="flex flex-col gap-2">
        <h2 className="font-pixel text-[10px] text-suave">DESAFIAR POR NICK</h2>
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 border-2 border-borda bg-painel px-3 py-2 text-[11px] text-texto outline-none focus:border-ciano"
          />
          <button
            type="button"
            onClick={handleBuscar}
            disabled={buscando || !busca.trim()}
            className="border-2 border-ciano px-4 py-2 font-pixel text-[9px] text-ciano transition hover:bg-ciano hover:text-fundo disabled:opacity-40"
          >
            {buscando ? "..." : "BUSCAR"}
          </button>
        </div>
        {erroBusca && <p className="text-[9px] text-rosa">{erroBusca}</p>}
        {achado && (
          <div className="flex items-center justify-between border-2 border-ciano bg-ciano/10 p-3">
            <div>
              <p className="font-pixel text-[10px] text-texto">{achado.nick}</p>
              <p className="mt-1 text-[9px] text-suave">
            ...
            <button
              ...
              className="border-2 border-rosa bg-rosa/10 px-4 py-2 font-pixel text-[9px] text-rosa transition hover:bg-rosa hover:text-fundo disabled:opacity-40"
            >
              DESAFIAR
            </button>
          </div>
      ...
      {/* ladder */}
      <section className="flex flex-col gap-2">
        <h2 className="font-pixel text-[10px] text-suave">LADDER · ADVERSÁRIOS</h2>
        {carregando ? (
          <p className="text-[10px] text-suave">Carregando…</p>
        ) : ladder.length === 0 ? (
          <p className="text-[10px] text-suave">Ninguém publicou snapshot ainda. Seja o primeiro — volte depois pra encontrar rivais.</p>
        ) : (
          ladder.map((op) => (
            <div key={op.userId} className="flex items-center justify-between border-2 border-borda bg-painel p-3">
              <div>
                <p className="font-pixel text-[10px] text-texto">{op.nick}</p>
                <p className="mt-1 text-[9px] text-suave">
                  {op.snapshot.rota} · {op.snapshot.elo}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-pixel text-[10px] text-rosa">⚔ {op.poder}</span>
                <button
                  type="button"
                  onClick={() => handleDesafiar(op)}
                  disabled={semCarreira || duelando}
                  className="border-2 border-rosa bg-rosa/10 px-3 py-1.5 font-pixel text-[8px] text-rosa transition hover:bg-rosa hover:text-fundo disabled:opacity-40"
                >
                  DUELAR
                </button>
              </div>
      ...
      {/* ranking */}
      <section className="flex flex-col gap-2">
        <h2 className="font-pixel text-[10px] text-suave">🏆 RANKING</h2>
        {ranking.length === 0 ? (
          <p className="text-[10px] text-suave">Ranking vazio — jogue duelos pra aparecer aqui.</p>
        ) : (
          ranking.slice(0, 10).map((r, i) => (
            ...
            >
              <span className="flex items-center gap-2 text-[10px] text-texto">
                <span className="font-pixel text-[9px] text-suave">{i + 1}º</span>
                {r.nick}
              </span>
              <span className="text-[9px] text-suave">
      ...
      {historico.length > 0 && (
        <section className="flex flex-col gap-2">
          <h2 className="font-pixel text-[10px] text-suave">HISTÓRICO</h2>
          {historico.map((h) => {
            ...
            return (
              ...
              >
                <span className="text-[10px] text-texto">vs {rival}</span>
                <span className={`font-pixel text-[8px] ${h.euGanhei ? "text-emerald-400" : "text-rosa"}`}>
