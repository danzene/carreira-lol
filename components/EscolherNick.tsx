  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-sm flex-col items-center justify-center gap-8 px-5 text-center">
      <div className="flex flex-col items-center gap-3">
        <span className="font-pixel text-[11px] tracking-[0.3em] text-ciano">SUA CONTA</span>
        <h1
        ...
        <p className="text-[12px] text-suave">É o nome que os outros jogadores vão ver no modo online.</p>
      </div>
      ...
      <form onSubmit={enviar} className="flex w-full flex-col gap-3 border-2 border-borda bg-painel p-5">
        ...
        />
        {erro && <p className="text-[12px] text-rosa">{erro}</p>}
        <button
          type="submit"
          disabled={salvando}
          className="border-2 border-ciano bg-ciano/10 py-2.5 font-pixel text-[11px] text-ciano transition hover:bg-ciano hover:text-fundo disabled:opacity-50"
        >
